...
export class UploadPreviewDocManager {
    constructor(inputSelector, previewSelector, triggerSelector, galleryOneDocManager = null) {
        ...
        this.galleryOneDocManager = galleryOneDocManager;
            ...
            if (this.files.length === 0 && !this.previewContainer.querySelector('.preview-item')) {
                ...
                this.previewContainer.innerHTML = `
                    <div class="col-12 d-flex justify-content-center align-items-center placeholder-preview">
                        <i class="fa-solid fa-file-signature"></i> ${emptyText}
                    </div>
                `;
            }
        }, { once: true });
    ...
    _onSubmit(e) {
        if (e.target.matches('#save_documents')) {
            e.preventDefault();
            this.save(e.target);
        }
    }
    ...
    _addPreview(fileBlob, existingId = null) {
        ...
        reader.onload = e => {
            ...
            col.className = 'col-6 col-md-4 mb-3'; // responsive
            col.dataset.id = id;
            col.innerHTML = `
               <div class="position-relative text-center preview-doc" data-id="${id}">
                    <div class="display-5 text-secondary mw-100 h-auto">
                        <i class="fa-solid fa-file-signature"></i>
                    </div>
                    <div class="small">${fileBlob.name}</div>
                        <button type="button"
    ...
    _removePreview(id) {
        ...
        if (el) {
            const col = el.closest('.col-6, .col-md-4');
    ...
    save(formEl) {
        ...
        apiFetch(urlbase + 'admin/uploadPreviewDoc/saveDocuments', {
            method: 'POST',
